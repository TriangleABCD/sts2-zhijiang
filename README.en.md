...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/Zhijiang/wiki/Home) | [English Home](https://github.com/alkaid616/Zhijiang/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `Zhijiang`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `Zhijiang.json` and update `id`, `name`, `author`, and `description`.
3. Edit `ZhijiangCode/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `Zhijiang/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `Zhijiang.json` is what the game loader checks at runtime. The template syncs the `STS2-RitsuLib` dependency version to the resolved NuGet version during build, but `min_game_version` still needs manual review.**
...
1. After build, confirm `dependencies[STS2-RitsuLib].version` in `Zhijiang.json` has been synced to the resolved `STS2.RitsuLib` version.
...
| Command | Behavior |
|---|---|
| `dotnet build .\Zhijiang.csproj` | Full build: compile + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**: copies the DLL and manifest to the game's `mods/Zhijiang` directory.
...
```text
Zhijiang/
├── ZhijiangCode/   # C# source
├── Zhijiang/       # Godot resources, localization, and placeholder scenes
├── Zhijiang.csproj
├── Zhijiang.json   # Mod manifest
├── project.godot
└── local.props.template
```

`res://Zhijiang/...` is the Godot/PCK resource path, mapping to the repository resource directory `Zhijiang/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
| Property | Value |
|---|---|
| Type | `ZhijiangCharacter` |
| Expected ID | `ZHIJIANG_CHARACTER_ZHIJIANG_CHARACTER` |
| Starter deck | 4 × `ZhijiangStrike`, 4 × `ZhijiangDefend`, 1 × `ZhijiangRelic` |
...
|---|---|---|
| `ZhijiangStrike` (attack) | character card pool | `ZHIJIANG_CARD_ZHIJIANG_STRIKE` |
| `ZhijiangDefend` (skill) | character card pool | `ZHIJIANG_CARD_ZHIJIANG_DEFEND` |
| `ZhijiangRelic` | `ZhijiangRelicPool` | `ZHIJIANG_RELIC_ZHIJIANG_RELIC` |

### Static placeholder assets

**Images** (`res://Zhijiang/images/...`):

- `cards/ZhijiangStrike.png`, `cards/ZhijiangDefend.png`: sample card art.
- `relics/ZhijiangRelic.png`: sample relic icon.
- `characters/Zhijiang_character_*.png`: character icons, select art, map marker, and energy icons.

**Scenes** (`res://Zhijiang/scenes/characters/...`):

| Scene | Purpose | Placeholder structure |
|---|---|---|
| `Zhijiang_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `Zhijiang_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `Zhijiang_merchant.tscn` | Merchant | — |
| `Zhijiang_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `Zhijiang_character_select_bg.tscn` | Character select background | — |
...
`Zhijiang.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:
